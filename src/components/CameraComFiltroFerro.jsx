...
const CameraComFiltro = () => {
  ...
  const gerarFotoFinal = (src) => {
    ...
    img.src = src;
    img.onload = () => {
      const hole = { x: 470, y: 550, width: 645, height: 720 };

      const imgAspect = img.width / img.height;
      const holeAspect = hole.width / hole.height;

      let drawWidth, drawHeight;
      const scaleMargin = 0.55;

      if (imgAspect > holeAspect) {
        drawHeight = hole.height * scaleMargin;
        drawWidth = drawHeight * imgAspect;
      } else {
        drawWidth = hole.width * scaleMargin;
      ...
      // Converte imagem desenhada em preto e branco
      const imageData = ctx.getImageData(hole.x, hole.y, hole.width, hole.height);
      const data = imageData.data;
      for (let i = 0; i < data.length; i += 4) {
        const avg = 0.299 * data[i] + 0.587 * data[i + 1] + 0.114 * data[i + 2]; // fórmula luminância
      ...
      moldura.src = process.env.PUBLIC_URL + "/pic2.png";
      moldura.onload = () => {
